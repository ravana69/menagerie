...
async function Draw(){
  if(!t){
    ...
    let animRl            = 0
    let animPt            = 0
    let animYw            = Math.PI/2
    let speed             = 1
    loadAnimation('tree_low_poly', animSize*.8, animX,animY,animZ,animRl,animPt,animYw, speed)
    loadAnimation('back_flip_male', animSize, animX,animY,animZ,animRl,animPt,animYw, speed)
    loadAnimation('walking_male', animSize, animX,animY,animZ,animRl,animPt,animYw, speed)
    loadAnimation('bug_lowpoly', animSize/4, animX,animY,animZ,animRl,animPt,animYw + Math.PI/2, speed)
    
    floorFunc = (X,Z) => {
      return S(X/(1600+C(t/4)*800)+t) * C(Z/800-t*4) * 800 + Math.max(1000, Math.min(1600, (.3+C(t/2))*3000))
    }
    
    floor = (X, Z) => {
      let d = Math.hypot(X, Z), p
      let facet = [],tx,ty,tz,val = floorFunc(X, Z)
      for(let i=3;i--;){
        tx = X+S(p=Math.PI*2/3*i)
        tz = Z+C(p)
        ty = floorFunc(tx,tz)
        facet = [...facet, [tx,ty,tz]]
      }
      return [val, Normal(facet, true, X, val, Z)]
    }
    ...
    loadLandscape = () => {
      ...
      for(i=iTreec;i--;){
        ...
        let pt = 0
        let yw = Rn()*Math.PI*2
        let animSel = landscape.length%animations.length
        let animation = JSON.parse(JSON.stringify(animations[animSel]))
        let speed = (1 + Rn()*2) * (animation.name.indexOf('bug')!=-1 ? 1.5: 1)
        let scale = (1+Rn()*3)/3
        let v = animation.name.indexOf('tree') == -1 && animation.name.indexOf('flip')==-1 ? iPv * speed * scale : 0
        landscape = [...landscape,
                       { X, Y, Z, v,
                         animation,
                         rl, pt, yw,
  ...
  if(showstars) ST.map((v, i) => {
    ...
    if(Z>0){
      d_ = Math.hypot(X,Y,Z)
      if((x.globalAlpha = Math.min(1,(d_/3000)**2*10 / (1+d_**8/1e27)))>.1){
        s = Math.min(1e4, 1e5/Z)
        l = Q()
        if(!(i%5)){
          x.drawImage(starImgs[1+7].img,l[0]-s/1.06,l[1]-s/1.06,s*2,s*2)
        }else{
          x.fillStyle = '#ffffff10'
  ...
  if(landScapeLoaded){
    ...
    if(drawFloor){
      ...
      for(let i=5e3;i--;){
        ...
        }while(Math.abs(oZ-lz)>landG);
       
        d_ = Math.hypot(oX-lx, oZ-lz)
        alpha = 1 / ((1+d_)**9/1e31)
        ...
        if(d_<4e3 && alpha>.1){
          ...
          if(0&&n<.5){
            ...
            R(Rl,Pt,Yw,1)
            if(Z>0) x.lineTo(...Q())
            X = lx + (l[1][3] - lx) * 400*(l_=.2+n*.8)
            Y = ly + (l[1][4] - ly) * 400*l_
            Z = lz + (l[1][5] - lz) * 400*l_
            R(Rl,Pt,Yw,1)
            if(Z>0) x.lineTo(...Q())
            stroke('#40f8','',300,false, alpha)
          }
          ...
          if(Z>0){
            d_ = 0
            landscape.map((tree,idx) => {
              d_ += 200000/((1+Math.hypot(tree.X-lx, tree.Z-lz))**3/6e3)
            })
            alpha2 = Math.min(2,d_/6)
            if(alpha2>.1){
              x.fillStyle = `hsla(${d_*20+t*500},99%,${Math.min(100,Math.max(0,40+d_*3))}%,1)`
              //x.globalAlpha = alpha * alpha2 *.06
              l = Q()
              s = Math.min(1e4,4e5/Z)
              //x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
              s/=3
              x.globalAlpha = alpha * alpha2 * .1
              x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
              s/=7
              x.globalAlpha = alpha * alpha2
    ...
    landscape.map((tree,idx) => {
      ...
      tree.X += S(tree.yw) * tree.v
      tree.Z += C(tree.yw) * tree.v
      
      do{
        tx = tree.X
        if(oX-tx > landG) tx = tree.X += landG*2
        if(oX-tx < -landG) tx = tree.X -= landG*2
      }while(Math.abs(oX-tx)>landG);
      
      do{
        tz = tree.Z
        if(oZ-tz > landG) tz = tree.Z += landG*2
        if(oZ-tz < -landG) tz = tree.Z -= landG*2
      }while(Math.abs(oZ-tz)>landG);
      
      let d_ = Math.hypot(oX-tx,(oZ)-tz)
      if(d_<4e3){
        ty = (l=floor(tx, tz))[0]
        if((alpha = 1 / ((1+d_)**9/1e31)) > .1){
          col1 = ''
          col2 = `hsla(${360/landscape.length*idx/1+t*(Math.min(10000,Math.max(50,(.3-C(t))*40000)))},99%,50%,.4)`
          //col2 = `hsla(${0},99%,50%,.3)`
          drawAnimation(tx, ty, tz, tree.animation, col1,col2, lineWidth=200, glowing=false, alpha, tree.speed, tree.scale, l[1], tree.yw)
        }
      }
    })
    x.globalAlpha = 1
